...
def read_trainingset(folderpath, d):
    tX = np.zeros([0, 1], dtype=np.int32)
    xX = np.zeros([0, T], dtype=np.int32)
    Y1 = np.zeros([0, 1], dtype=np.int32)
...
for d in xrange(1, 32):
    ...
    tX_all = np.zeros([0, 1], dtype=np.int32)
    xX_all = np.zeros([0, T], dtype=np.int32)
    Y1_all = np.zeros([0, 1], dtype=np.int32)
    w_all = np.zeros([0, 1], dtype=np.float)
    tX, xX, Y1 = read_trainingset('/home/fan/work/data/dis_forensemble_2011_jan/', d)
